...
(() => {
  'use strict';
  const G = window.ForestGame, W = window.ForestWorld, J = window.ForestJourney, V = window.ForestVillage, VV = window.ForestVillageView, {icon, petSVG} = window.ForestArt;
  ...
  function character(c) { return W.personality(c).name; }
  function speech(c) {
  ...
  function trailsCard() {
    const c=G.active(state),visited=G.visitedRoutes(c);
    return `<section class="card trail-collection"><div class="section-heading"><h2>${esc(c.name)}の旅の手帖</h2><span>${visited.length}/4</span></div><p>最後まで歩いた道を、この子だけの記録に。</p><div class="trail-stamps">${Object.entries(G.ROUTES).map(([id,r])=>`<div class="trail-stamp ${visited.includes(id)?'visited':''}">${icon(r.icon)}<strong>${r.name}</strong><span>${visited.includes(id)?'歩いた道':'これからの道'}</span></div>`).join('')}</div><button class="button wide" data-view="explore">次のお出かけを考える</button></section>`;
  }
  function journalView() {
    return `<div class="page-heading"><p class="eyebrow">THE DAYS WE SHARED</p><h1>一緒に過ごした日々</h1><p>できたことも、何気ないひと言も。新しい順に150件を記録します。</p></div>${trailsCard()}${albumCard()}<div class="journal-layout"><section class="card journal-card"><h2>はじまりの手帖</h2>${G.QUESTS.map(q=>`<div class="achievement ${state.quests.includes(q.id)?'complete':''}"><span aria-hidden="true">${state.quests.includes(q.id)?'✓':'○'}</span><div><h3>${q.name}</h3><p>${q.detail}</p></div><b>${q.reward} G</b></div>`).join('')}</section><section class="card journal-card"><h2>思い出の記録</h2><ol class="timeline">${state.logs.map(l=>`<li><time>${new Date(l.at).toLocaleDateString('ja-JP',{month:'short',day:'numeric'})}</time><p>${esc(l.text)}</p></li>`).join('')}</ol></section></div>`;
  }
  function render() {
    if (!state) return;
  ...
  function talkDialog() {
    ...
    modal(`${esc(c.name)}と、ことばの時間`,`<div class="talk-friend">${petSVG(c)}<div class="speech-note" aria-live="polite">${esc(c.reply)}</div></div><form id="talk-form"><label class="field-label" for="talk-input">どんな言葉を届けよう？</label><div class="talk-input-wrap"><input id="talk-input" maxlength="140" placeholder="例：今日もよく頑張ったね" value="${esc(draft)}" autocomplete="off" enterkeyhint="send" ${blocked?'disabled':''}><button type="submit" aria-label="言葉を届ける" ${blocked?'disabled':''}>${icon('send')}</button></div></form><div class="suggestions">${['よく頑張ったね','大好きだよ','調べてみよう'].map(t=>`<button data-say="${t}" ${blocked?'disabled':''}>${t}</button>`).join('')}</div><p class="talk-note">性格の成長は1日3回まで。同じ言葉は1日1回。<br>その後も、何度でも声をかけられます。<br>言葉の特徴とこの子の性格で返事が変わる簡易会話です。生成AIとの自由会話ではありません。</p>`,'talk');
  }
  ...
  function routeArt(id) {
    const colors={forest:['#dce9c8','#92b382','#547b50'],meadow:['#eef0c8','#bdd38d','#6e985c'],brook:['#deedf0','#a0c8b1','#567f65'],ridge:['#eee1d0','#baaf94','#777d66']};
    const [sky,hill,front]=colors[id];
    const detail=id==='brook'?'<path d="M100 28q-38 15 2 26t-25 27" stroke="#7fb1c0" stroke-width="18" fill="none"/>':id==='meadow'?'<g fill="#fff9d6"><circle cx="39" cy="57" r="4"/><circle cx="66" cy="65" r="4"/><circle cx="133" cy="54" r="4"/></g>':id==='ridge'?'<g fill="#fff9df"><circle cx="33" cy="14" r="3"/><circle cx="120" cy="13" r="2"/><path d="m153 15 2 5 5 2-5 2-2 5-2-5-5-2 5-2Z"/></g>':'<path d="m32 56 14-35 14 35Zm78 1 15-43 16 43Z" fill="#466c43"/>';
    return `<svg viewBox="0 0 180 80" class="route-art" aria-hidden="true"><rect width="180" height="80" rx="12" fill="${sky}"/><path d="M0 59 34 23 76 56 117 28 180 62v18H0Z" fill="${hill}"/><path d="M0 72q47-26 95-7t85-6v21H0Z" fill="${front}"/>${detail}</svg>`;
  }
  function routesDialog() {
    const c=G.active(state),visited=G.visitedRoutes(c);
    modal('今日は、どの道を歩こう？',`<p>行き先を選ぶと出発します。1体につき全コース共通で1日1回。どの道も3場面、制限時間はありません。</p><div class="route-list">${Object.entries(G.ROUTES).map(([id,r])=>`<button class="route-option" data-route="${id}">${routeArt(id)}<span class="route-description"><strong>${r.name} ${visited.includes(id)?'<small>訪問ずみ</small>':''}</strong><span>${r.summary}</span><small>育つ力：${r.focus}</small></span><span aria-hidden="true">→</span></button>`).join('')}</div><p class="route-note">能力が足りない選択でも、おみやげと経験は残ります。選ぶ前に、この子の能力と結果の目安を確認できます。</p>`,'routes');
  }
  function adventure(route=null) {
    if(!state.adventure){if(!route){routesDialog();return;}const result=G.beginAdventure(state,route);if(!result.ok){toast(result.message);return;}save();render();}
    const run=state.adventure,c=state.creatures.find(c=>c.id===run.petId),complete=run.step>=3,e=complete?null:G.currentEncounter(state);
    modal(complete?'小さな冒険、大成功。':e.title,`<p>${esc(c.name)} · ${G.ROUTES[G.adventureRoute(state)].name} · ${complete?'帰り道':`${run.step+1}/3`}</p><div class="adventure-steps">${[0,1,2].map(n=>`<span class="${n<run.step?'done':''}"></span>`).join('')}</div><div class="adventure-scene">${icon(complete?'gift':e.icon)}${petSVG(c)}</div>${run.lastText?`<p class="adventure-message">${esc(run.lastText)}</p>`:''}<p>${complete?'森の出口から、おうちが見えてきました。今日の宝物を持って帰ろう。':e.text}</p>${complete?`<div class="loot"><strong>${run.coins} G</strong><strong>${run.xp} XP</strong></div><button class="button primary wide" data-action="finish-adventure">おうちに帰る ${icon('home')}</button>`:`${e.choices.map((o,i)=>{const p=G.previewChoice(state,i);return `<button class="choice-button" data-choice="${i}"><strong>${o.label} →</strong><span>${o.hint || `${G.ABILITIES[o.ability]}が育つ選択`}</span><span class="choice-outlook">${G.ABILITIES[p.ability]} ${p.value.toFixed(1)}${p.threshold>0?` / 目安 ${p.threshold} · ${p.success?'得意な選択':'挑戦になる選択'}`:' · 確実なおみやげ'}<br>${p.coins} G · ${p.xp} XP · ${G.ABILITIES[p.ability]} +1.2</span></button>`;}).join('')}<div class="modal-actions"><button class="button" data-action="retreat">ここまでで帰る</button><button class="button" data-action="close">あとでつづける</button></div>`}`,'adventure');
  }
  function nextDay() {
  ...
  document.addEventListener('click',event=>{
    ...
    if(b.dataset.route){adventure(b.dataset.route);return;}
    ...
    if(action==='pet'){if(G.active(state).dead||state.adventure)return;J.mark(state,'care');const c=G.active(state);c.reply=W.response(c,'pet');outcome({ok:true,message:`${c.name}：${c.reply}`},'talk');}
