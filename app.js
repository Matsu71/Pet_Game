...
(() => {
  ...
  let kind = '', opener = null, pendingImport = null, toastTimer, audio;
  const corrupt = new Set(), preferences = {sound: false};
  function read(name) {
    let raw;
    try { raw = localStorage.getItem(KEY(name)); } catch { saveOK = false; return null; }
    if (!raw) return null;
    try { const s = JSON.parse(raw); if (valid(s) && s.mode === name) return s; } catch { /* Back up before replacing. */ }
    corrupt.add(name); return null;
  }
  function save() {
    if (!state) return;
    J.ensure(state);
    try {
      ...
      localStorage.setItem(KEY(state.mode), JSON.stringify(state)); localStorage.setItem(LAST_MODE, state.mode); saveOK = true;
    } catch { saveOK = false; }
  ...
  function start(targetMode) {
    ...
    state=G.fresh(targetMode,name,selectedSpecies); mode=targetMode; draft=''; save();closeModal();navigate('home');toast(`${name}との暮らしが、はじまりました。ごはんをどうぞ。`);
  }
  function talkDialog() {
  ...
  document.addEventListener('click',event=>{
    ...
    if(b.dataset.decor){outcome(J.decorate(state,b.dataset.decor));decorations();return;}
    if(b.dataset.memory!==undefined){const result=J.flip(state,Number(b.dataset.memory));save();render();memoryDialog();if(result.message)toast(result.message);if(result.matched||result.complete)sound();return;}
    ...
    if(action==='memory-hide'){J.hideCards(state);save();memoryDialog();}
    ...
    if(action==='confirm-reset'){const oldMode=state.mode;try{localStorage.removeItem(KEY(oldMode));}catch{toast('保存領域を消せませんでした。新しい村は、この回だけになる場合があります。');}state=null;$('main-content').innerHTML='';welcome(oldMode);}
    if(action==='confirm-import'){const imported=pendingImport;if(!imported)return;pendingImport=null;state=imported;mode=state.mode;corrupt.delete(mode);G.sync(state);save();closeModal();draft='';navigate('home');toast('セーブデータを読み込みました。おかえりなさい。');}
  });
  ...
  document.addEventListener('change',async event=>{
    if(event.target.id==='job-select'){G.sync(state);outcome(G.setJob(state,event.target.value));profile();$('job-select')?.focus({preventScroll:true});}
    if(event.target.id!=='save-file')return;
    const file=event.target.files?.[0];event.target.value='';if(!file)return;
    try{if(file.size>2e6)throw new Error('size');const candidate=JSON.parse(await file.text());if(!valid(candidate))throw new Error('schema');pendingImport=candidate;modal('この村を、読み込みますか？',`<p>${candidate.mode==='demo'?'おためし':'通常'}モードの村を置き換えます。<br>仲間 ${G.alive(candidate).length}体 / ${candidate.coins} G${candidate.mode==='real'?'<br>通常モードは保存後の時間も反映します。':''}</p><div class="modal-actions"><button class="button" data-action="settings">やめる</button><button class="button primary" data-action="confirm-import">読み込む</button></div>`,'import');}catch{pendingImport=null;toast('読み込めませんでした。森の子の有効なセーブデータを選んでください。');}
  });
  ...
  if(state){G.sync(state);render();save();}else welcome();
  ...
  window.addEventListener('storage',event=>{if(!state||event.key!==KEY(state.mode))return;try{if(!event.newValue){state=null;$('main-content').innerHTML='';welcome(mode);return;}const loaded=JSON.parse(event.newValue);if(!valid(loaded)||loaded.mode!==state.mode)return;state=loaded;G.sync(state);closeModal();render();toast('別のタブの変更を反映しました。');}catch{ /* Never overwrite valid local state with malformed storage events. */ }});
})();
